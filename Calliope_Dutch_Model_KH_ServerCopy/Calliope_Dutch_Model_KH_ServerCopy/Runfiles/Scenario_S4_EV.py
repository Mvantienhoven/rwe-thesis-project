...
base_dir = Path.cwd()  
MODEL_FILE = base_dir / "Research_Runs" / "Scenario_S4.yaml"
OUTPUT_DIR = base_dir / "output"    / "5.Scenario_S4"
LOG_VERBOSITY = "INFO" 
...
if __name__ == "__main__":
    ...
    yaml_file = create_versioned_filename("S4_EV_10K" + scenario_string, "yaml")
    model.save_commented_model_yaml(yaml_file)
    print(f" Fully merged model YAML saved to {yaml_file}")


    nc_file = create_versioned_filename("S4_EV_10K" + scenario_string, "nc")
    model._model_data.to_netcdf(str(nc_file))
