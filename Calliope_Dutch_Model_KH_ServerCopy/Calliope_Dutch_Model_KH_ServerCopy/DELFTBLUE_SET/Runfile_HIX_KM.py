...
base_dir = Path.cwd()  
MODEL_FILE = base_dir / "Research_Runs" / "Scenario_HIX.yaml"
...
if __name__ == "__main__":
    ...
    yaml_file = create_versioned_filename("HIX_KM" + scenario_string, "yaml")
    model.save_commented_model_yaml(yaml_file)
    print(f" Fully merged model YAML saved to {yaml_file}")

    results = model.backend.rerun()

    nc_file = create_versioned_filename("HIX_KM" + scenario_string, "nc")
