...
base_dir = Path.cwd()  
MODEL_FILE = base_dir / "Research_Runs" / "Scenario_RRX_C150.yaml"
OUTPUT_DIR = base_dir / "output"    / "RollsRoyce"
LOG_VERBOSITY = "INFO" 
...
tech_list =  ["pp_SMR_RollsRoyce_CHH2P_1"]


##FOR UNITS
from pyomo.environ import Var, Constraint, NonNegativeReals

def inject_trapezoid_constraints(model, tech_list, tol=1e-4):
    """
...
if __name__ == "__main__":
    ...
    inject_trapezoid_constraints(model, tech_list, tol=1e-4)
    ...
    yaml_file = create_versioned_filename("RRX_C150" + scenario_string, "yaml")
    model.save_commented_model_yaml(yaml_file)
    print(f" Fully merged model YAML saved to {yaml_file}")

    results = model.backend.rerun()

    nc_file = create_versioned_filename("RRX_C150" + scenario_string, "nc")
